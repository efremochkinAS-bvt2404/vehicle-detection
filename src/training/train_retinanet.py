...
from src.evaluation.retinanet import collect_predictions, evaluate_retinanet
from src.models.retinanet.model import build_retinanet, save_checkpoint
...
def train_retinanet(verbose=True):
    ...
    experiment = create_experiment("retinanet", config)

    if verbose:
        print("=" * 50, flush=True)
        print("RetinaNet training started", flush=True)
        print("=" * 50, flush=True)
    ...
    if verbose:
        print()
        print("=" * 50, flush=True)
        print("RetinaNet training completed", flush=True)
        print("=" * 50, flush=True)
        ...
        print()
        print("=" * 50, flush=True)
        print("Running RetinaNet test evaluation", flush=True)
        print("=" * 50, flush=True)

    evaluate_retinanet(experiment_dir=experiment.root_dir, verbose=verbose)

    return experiment.root_dir


def main():
    train_retinanet(verbose=True)
