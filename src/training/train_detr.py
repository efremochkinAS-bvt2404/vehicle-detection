...
from src.evaluation.detr import collect_predictions, evaluate_detr
from src.models.detr.model import build_detr_model, build_detr_processor, save_checkpoint
...
def create_detr_dataloaders(config, image_processor):
    training_config = config["training"]
    augmentation_config = config.get("augmentation", {})

    train_dataset = DetrDetectionDataset(
    ...
    val_dataset = DetrDetectionDataset(
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        if not torch.isfinite(loss):
            print(
                ...
                f"non-finite loss detected, skipping batch",
                flush=True,
            )
...
def train_detr(verbose=True):
    ...
    experiment = create_experiment("detr", config)

    if verbose:
        print("=" * 50, flush=True)
        print("DETR training started", flush=True)
        print("=" * 50, flush=True)
    ...
    train_loader, val_loader = create_detr_dataloaders(config, image_processor)
    backbone_parameters = []
    other_parameters = []
    ...
    if verbose:
        print()
        print("=" * 50, flush=True)
        print("DETR training completed", flush=True)
        print("=" * 50, flush=True)
        ...
        print()
        print("=" * 50, flush=True)
        print("Running DETR test evaluation", flush=True)
        print("=" * 50, flush=True)

    evaluate_detr(experiment_dir=experiment.root_dir, verbose=verbose)

    return experiment.root_dir


def main():
    train_detr(verbose=True)
