...
from src.models.ssd.model import load_model_from_checkpoint
...
def evaluate_ssd(experiment_dir=None, verbose=True):
    ...
    experiment_dir = (
        ...
        else find_latest_experiment("ssd")
    )
    ...
    if verbose:
        print("=" * 50, flush=True)
        print("SSD evaluation started", flush=True)
        print("=" * 50, flush=True)
    ...
    if verbose:
        print()
        print("=" * 50, flush=True)
        print("SSD evaluation completed", flush=True)
        print("=" * 50, flush=True)
